...
detailsRoutes.route('/add').post(function (req, res) {
  ...
  business.save()
    .then(business => {
      res.status(200).json({'business': 'data in added successfully'});
    })
    .catch(err => {
...
detailsRoutes.route('/update/:id').post(function (req, res) {
    CrudDetails.findById(req.params.id, function(err, business) {
    if (!business)
...
detailsRoutes.route('/delete/:id').get(function (req, res) {
    CrudDetails.findByIdAndRemove({_id: req.params.id}, function(err, business){
        if(err) res.json(err);
